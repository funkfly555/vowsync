/**
 * ColumnFilterDropdown - Excel-style column filtering with checkboxes for items
...
import type { ItemTableColumnDef, ItemTableRow, ItemColumnFilter } from '@/types/item-table';

interface ColumnFilterDropdownProps {
  column: ItemTableColumnDef;
  data: ItemTableRow[];
  activeFilter?: ItemColumnFilter;
  onFilterChange: (filter: ItemColumnFilter | null) => void;
}
 ...
 */
function formatValueForDisplay(value: unknown, column: ItemTableColumnDef): string {
  ...
  switch (column.type) {
    case 'badge':
      ...
      if (typeof value === 'string') {
        // Handle availability_status values
        if (value === 'sufficient') return 'Sufficient';
        if (value === 'shortage') return 'Shortage';
        if (value === 'unknown') return 'Unknown';
        // Handle aggregation_method values
        if (value === 'ADD') return 'ADD (Sum)';
        if (value === 'MAX') return 'MAX (Maximum)';
        return value;
      }
      return String(value);
...
function getUniqueValues(
  ...
  for (const row of data) {
    ...
    const normalizedValue = getNormalizedValue(rawValue);
    const displayValue = formatValueForDisplay(rawValue, column);
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <button
          className={cn(
            'p-0.5 rounded transition-colors text-white/70 hover:text-white hover:bg-white/20',

...
import type { ColumnFilter, TableColumnDef, MealOptionLookup } from '@/types/guest-table';
  ...
  switch (column.type) {
    ...
    case 'meal':
      if (mealOptions && column.courseType) {
        ...
        const mealName = mealOptions[column.courseType]?.[numValue];
 ...
 */
function getColumnDisplayName(column: TableColumnDef): string {
  if (column.eventName) {
    return `${column.eventName}: ${column.header}`;
  }
    ...
    <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 border-b border-gray-200 flex-wrap">
      ...
      {filters.map((filter) => {
        ...
        const displayValues = selectedValues
          ...
          .map((v) => formatFilterValue(v, column, mealOptions))
          .join(', ');

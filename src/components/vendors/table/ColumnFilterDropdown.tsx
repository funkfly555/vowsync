...
interface ColumnFilterDropdownProps {
  ...
  data: VendorTableRow[];
  activeFilter?: VendorColumnFilter;
  onFilterChange: (filter: VendorColumnFilter | null) => void;
}
...
function formatValueForDisplay(
  ...
  switch (column.type) {
    ...
    case 'masked':
      return maskAccountNumber(value as string) || '(Empty)';
 ...
 */
function getNormalizedValue(value: unknown, column: VendorTableColumnDef): string {
  if (value === null || value === undefined) {
    return '__null__';
  }
...
function getUniqueValues(
  ...
  const values = Array.from(valueMap.entries()).map(
    ([value, { display, count }]) => ({
      value,
      display,
      count,
...
export function ColumnFilterDropdown({
  ...
  const someSelected =
    filteredValues.some((v) => selectedValues.has(v.value)) && !allSelected;
          ...
          {filteredValues.length === 0 ? (
            ...
            filteredValues.map((item) => (
              <div
                ...
                onClick={() =>
                  handleValueToggle(item.value, !selectedValues.has(item.value))
                }
              >
                <div className="flex items-center gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleValueToggle(item.value, !!checked)
                    }
        ...
        {hasActiveFilter && (
          <div className="px-3 py-2 border-t border-gray-200 bg-gray-50">
            <div className="flex items-center gap-1 text-xs text-gray-500">
              ...
              <span>
                {selectedValues.size} value
                {selectedValues.size !== 1 ? 's' : ''} selected
              </span>
            </div>
          </div>
        )}
